...
var login = function(done) {
  async.series([
    ...
    debugCapture('login2'),
    setInput('#txtPassword', {name: 'pass', hidden: true}),
    debugCapture('login3'),
...
var goToOrderPage = function(done) {
  async.series([
    ...
    $.go(false, 'visit', '/glitter/order-menu.asp?t=0&str_date=20141023'),
    $.go(false, 'waitForElement', '.order-category'),
...
var selectSandwich = function(done) {
  ...
  if (!sandwich) {
    ...
    var found = false, selection = 0, query = '.order-line em';

    // Iterate over each sandwich.
    $(query).each(function(index, item, done) {
      item.attr('title', function(title) {
        if (title.toLowerCase().indexOf(sandwich) != -1) {
          selection = index;
          found = true;
        }
        done();
